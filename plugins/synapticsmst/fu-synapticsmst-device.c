...
static gboolean
fu_synapticsmst_device_open (FuDevice *device, GError **error)
{
	FuSynapticsmstDevice *self = FU_SYNAPTICSMST_DEVICE (device);

	/* open device */
	self->fd = g_open (fu_udev_device_get_device_file (FU_UDEV_DEVICE (device)), O_RDWR);
	if (self->fd < 0) {
		g_set_error (error,
			     G_IO_ERROR,
			     G_IO_ERROR_FAILED,
...
static gboolean
fu_synapticsmst_device_close (FuDevice *device, GError **error)
{
	FuSynapticsmstDevice *self = FU_SYNAPTICSMST_DEVICE (device);
	if (self->fd == 0)
		return TRUE;
					 ...
					 GError **error)
{
	...
	if ((flags & FWUPD_INSTALL_FLAG_FORCE) == 0 &&
		...
		//board_id = (*(buf + ADDR_CUSTOMER_ID) << 8) + *(buf + ADDR_BOARD_ID);
		board_id = fu_common_read_uint16 (buf + ADDR_CUSTOMER_ID, G_LITTLE_ENDIAN);
...
static gboolean
fu_synapticsmst_device_scan_cascade (FuSynapticsmstDevice *self, GError **error)
{
	...
	for (guint8 j = 0; j < 2; j++) {
		...
		guint16 rad = self->rad | (j << (2 * (layer - 1)));
		guint8 byte[4];
		g_autoptr(FuSynapticsmstConnection) connection = NULL;
		g_autoptr(GError) error_local = NULL;
		...
		self->layer = layer;
		self->rad = rad;
		if (!fu_synapticsmst_device_scan_cascade (self, error))
			return FALSE;
	}
	return TRUE;
}
...
fu_synapticsmst_device_rescan (FuDevice *device, GError **error)
{
	...
	/* at a dummy GUID so the device always gets added to the daemon */
	fu_device_add_instance_id (FU_DEVICE (self), "DRM_DP_AUX_DEV");
	...
	if (!fu_synapticsmst_device_read_board_id (self, connection, buf_ver, error))
		return FALSE;
	self->board_id = (buf_ver[0] << 8) | (buf_ver[1]);
	...
	if (g_strcmp0 (guid_template, "system") == 0) {
		//const gchar *system_type = fu_plugin_get_dmi_value (plugin, FU_HWIDS_KEY_PRODUCT_SKU);
		const gchar *system_type = "FIXME";
...
fu_synapticsmst_device_class_init (FuSynapticsmstDeviceClass *klass)
{
	...
	klass_device->open = fu_synapticsmst_device_open;
	klass_device->close = fu_synapticsmst_device_close;
